...
export default function Technique({
  ...
  function handleAddNote() {
    const newNotes = [
      ...technique.notes,
      {
        noteText: "Add note here.",
        noteID: technique.notes.length + 1,
        noteTitle: "Note " + (technique.notes.length + 1),
      },
    ];
  ...
  return (
    <div
      ...
      id={technique.id}
      onTouchMove={handleTouchDragStart}
    ...
    >
      ...
      <button onClick={handleAddNote} id="add">
        +
      </button>
      <button onClick={() => handleDeleteTechnique(technique.id)} id="delete">
        Delete
      </button>
    </div>
  );
